...
public partial class Rule3D<T>
{
	...
	public Rule3D( int matchRadius, Vector2I position, in List<List<T>> sample, T zeroValue )
	{
		this.zeroValue = zeroValue;
		CreateRuleArray( matchRadius );
		for ( int i = 0; i < RuleArray.Count; i++ )
	...
	public Rule3D( int matchRadius, Vector2I position, in List<List<T>> tilesArray,  T zeroValue, bool offsetPos=true )
	{
		this.zeroValue = zeroValue;
		CreateRuleArray( matchRadius );
		for ( int i = 0; i < RuleArray.Count; i++ )
